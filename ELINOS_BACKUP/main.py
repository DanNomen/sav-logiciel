...
# Global flag and lock to prevent multiple concurrent syncs
IS_SYNCING = False
SYNC_LOCK = threading.Lock()

def sync_vrm_data():
    """
    Fetches installations from VRM and adds them to the SYSTEMS list incrementally.
    """
    global IS_SYNCING
    with SYNC_LOCK:
        if IS_SYNCING:
            print("[SYNC] Sync already in progress, skipping startup call...")
            return 0
        IS_SYNCING = True

    print("[SYNC] Starting incremental VRM Sync...")
    try:
        from victron_api.api_vrm import VictronVRM
        vrm = VictronVRM()
        
        # 1. Fetch Basic List
        installations = vrm.get_all_installations()
        if not installations:
            print("[SYNC] No installations found in VRM.")
            return 0
            
        print(f"[SYNC] Found {len(installations)} installations. Enriching details...")
        
        # 2. Ensure Default Client Exists
        ...
        if not default_client_id:
            ...
            new_client = {
                ...
                "categorie": "Silver",
                "localisation": "Automatique via VRM",
        ...
        for inst in installations:
            ...
            exists = False
            for s in SYSTEMS:
                if s.get("victron_site_id") == site_id:
                    exists = True
                    break
            ...
            # Fetch details for this specific site
            sys_data = vrm.format_installation_for_system_form(inst)
            print(f"  -> Enriching {name} (ID: {site_id})...")
            sys_data = vrm.enrich_system_with_devices(sys_data, site_id)
            
            sys_data["id"] = str(uuid.uuid4())
            sys_data["client_id"] = default_client_id
            
            # Append immediately to the global list
            SYSTEMS.append(sys_data)
            count += 1
                
        print(f"[SYNC] VRM Sync Complete. Added {count} new systems.")
        return count
    except Exception as e:
        print(f"[SYNC] Error during VRM Sync: {e}")
        import traceback
        traceback.print_exc()
        return 0
    finally:
...
async def startup_event():
    import threading
    # Run the sync in a separate thread to not block startup
    print("Application startup: Starting VRM Sync in background...")
...
def sync_systems_endpoint():
    global IS_SYNCING
    if IS_SYNCING:
        return {"message": "La synchronisation est déjà en cours dans le backend.", "status": "syncing"}
    ...
    return {"message": "Synchronisation VRM lancée en arrière-plan. Cela peut prendre quelques minutes.", "status": "started"}
